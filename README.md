...
- **多會話管理**：支持創建多個聊天會話，並可透過**資料夾**進行分類管理。
- **智能標題**：使用 AI 自動為您的對話生成簡短且貼切的標題。
- **個性化設定**：
    - **UI 優化**：預設採用 **Claude 風格** 的極簡介面，側邊欄資料夾採用 **ChatGPT 風格** 的無框設計，視覺更清爽。
    - **主題切換**：內建 Claude (預設), Dark, Light, Cyberpunk, Nature 等多種主題。
    - **頭像選擇**：可自由選擇使用者頭像。
    - **使用者名稱**：可自定義稱呼。
- **訊息編輯**：支援編輯已發送的訊息，方便修正錯誤。
- **Markdown 支援**：完美支援 Markdown 語法與程式碼區塊顯示。
